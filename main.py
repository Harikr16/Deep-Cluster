...
def main():
	...
	for epoch in range(args.start_epoch, args.epochs):
		...
		if epoch%args.update_loader == 0:
			dataset,dataloader,batch_size = update_loader(loader_update_count,dataset, dataloader)
			loader_update_count+=1
		...
		# features = np.random.randn(268243,1920)
...
def update_loader(loader_update_count,dataset = None, dataloader = None):
	res_list = [224,360,512] #
	bsize_list = [36,14,28]	#
	if loader_update_count<len(res_list):
		resolution = res_list[loader_update_count]
		batch_size =bsize_list[loader_update_count]
		normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225])

		tra = [Resize(resolution),transforms.ToTensor(),normalize, transforms.Lambda(lambda x: x.half())]
		dataset = datasets.ImageFolder(args.data , transform=transforms.Compose(tra)) 
		# batch_size = int(512*512*args.batch/resolution/resolution)
		b_size_res = 256
		print("Feature Computation Batch Size : ", b_size_res)
		print("Epoch training Batch Size : ", batch_size)
		if args.verbose: print('Dataset resolution : {0:.2f}*{0:.2f}'.format(resolution))
		dataloader = torch.utils.data.DataLoader(dataset,
											 batch_size=b_size_res,
											 num_workers=args.workers,
											 pin_memory=True)
	else:
		batch_size = bsize_list[-1]
	# pdb.set_trace()
